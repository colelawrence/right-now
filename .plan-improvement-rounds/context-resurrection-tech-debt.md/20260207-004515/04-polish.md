...
- Rust: add request timeout handling in `daemon_client.rs`:
  - default timeout: 5 seconds for IPC requests
  - surface `timeout` error code to TS client
  - distinguish unreachable daemon (connect failure → `store_unavailable`) from slow daemon (timeout → `timeout`)
- TS: update `CrClient` to map by `code` (no string matching).
- TS: define retry policy in `client.ts`:
  - retryable codes: `timeout`, `store_unavailable` (max 1 retry, fixed 2s delay before retry)
  - non-retryable: `not_found`, `skipped`, `invalid_request`, `internal`
- Tests:
...
- **Snapshot list pagination (defer complexity, enforce limits):**
  - Add `limit` param to `CrGetSnapshots` request (default 100, max 500).
  - Response returns at most `limit` most-recent snapshots per task.
...
- Implement option 2 (split runtime vs persistent dirs):
  ...
  - `runtime_dir`: socket/pid files (Linux: `XDG_RUNTIME_DIR/right-now`, macOS: `~/.right-now`).
  - `state_dir`: persistent data including CR snapshots (all platforms: `~/.right-now`).
  - Ensure CR snapshots always go to `state_dir`.
...
- Storage file permissions + locking:
  - Create snapshot directory (`state_dir/cr/`) with `0700` permissions.
...
- Operability: document reset procedure in `docs/context-resurrection.md`:
  - Clear instructions: "To reset CR state: stop daemon, delete `~/.right-now/cr/` (or state_dir), restart daemon."
